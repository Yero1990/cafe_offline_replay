void make_plots_coin(){
  ...
  TString data_fname="~/ROOTfiles/optics_aug14/no_angle_offset/cafe_replay_optics_16962_-1.root"; // previous  optics
  
  TString simc_fname="~/ROOTfiles/optics_aug14/no_angle_offset/cafe_heep_coin_kin0_rad.root";  // no hms collimator interaction (set to 0 in input file)
  ...
  //TString simc_fname="~/ROOTfiles/heep_coin_optim/step1/cafe_heep_coin_kin0_rad_0p0_hMispointPos_eMispointNeg.root"; // hytar_aligned, hyptar_aligned, hxptar_simc>hxptar_data,
  //e_ytar_simc>e_ytar_data, eyptar_aligned, exptar_alinged, tarz_simc < tarz_data

  //TString simc_fname="~/ROOTfiles/heep_coin_optim/step1/cafe_heep_coin_kin0_rad_0p0_hMispointPos_eMispointXNegYPos.root"; // e_ytar still off, as well as tar_z

  //TString simc_fname="~/ROOTfiles/heep_coin_optim/step1/cafe_heep_coin_kin0_rad_0p0_hMispointPos_NOeMispoint.root";

  // e- angle:  (run 20861)
  //TString data_fname="~/ROOTfiles/heep_coin_optim/step4/cafe_replay_optics_20861_100000.root";
  //TString simc_fname="~/ROOTfiles/heep_coin_optim/step4/d2_heep_scan_coin_rad_+8.root";  
  ...
  for(int i=0; i<nplots; i++){
    ...
    //plot only kinematics (kf, th_e, Q2, xbj, nu, W, Em, Pmx,y,z, Pm )
    if( (i!=0) &&  (i!=6) && (i!=7) && (i!=8) && (i!=9) &&  (i!=10) && (i!=20) &&  (i!=21) && (i!=22) && (i!=23) && (i!=24) && (i!=25) && (i!=27) &&  (i!=29) ) continue;
    ...
    if(i==8) {
      c[i] = new TCanvas(Form("c%i",i), "", 900, 700);
      H_data[i]=new TH1F("H_Q2", "Q2", 200, 0,4);
      H_simc[i]=new TH1F("H_Q2_simc", "Q2", 200, 0,4);
      H_simc[i]->SetLineColor(kRed);
      fdata->cd();
      T->Draw("P.kin.primary.Q2>>H_Q2(200,0,4)", data_cuts, "normhistE");
      fsimc->cd();
      SNT->Draw("Q2>>H_Q2_simc(200,0,4)", simc_cuts, "normhistEsames");
    }

    if(i==9) {
      c[i] = new TCanvas(Form("c%i",i), "", 900, 700);
      fdata->cd();
      T->Draw("P.kin.primary.scat_ang_deg>>H_the(100,7.5,9.)", data_cuts, "normhistE");
      fsimc->cd();
      SNT->Draw("theta_e*180/3.14>>H_the_simc(100,7.5,9.)", simc_cuts, "normhistEsames");
  ...
  if(i==25) {
    ...
    if(i==27) {
      c[i] = new TCanvas(Form("c%i",i), "", 900, 700);
      fdata->cd();
      T->Draw("H.kin.secondary.pmiss>>H_Pm(100,-0.01,0.05)", data_cuts, "normhistE");
      fsimc->cd();
      SNT->Draw("Pm>>H_Pm_simc(100,-0.01,0.05)", simc_cuts, "normhistEsames");
    ...
    if(i==28) {
      ...
      fdata->cd();
      c1_recon_corr->cd(1);
      T->Draw("P.kin.primary.W:P.gtr.th>>H_shms_W_vs_xptar(100, -0.015, 0.015, 100,0.9,1.)", data_cuts, "colz");  
      c1_recon_corr->cd(2);
      T->Draw("P.kin.primary.W:P.gtr.ph>>H_shms_W_vs_yptar(100, -0.015, 0.015, 100,0.9,1.)", data_cuts, "colz");  
      c1_recon_corr->cd(3);
      T->Draw("P.kin.primary.W:P.gtr.y>>H_shms_W_vs_ytar(100, -1.5, 1.5, 100,0.9,1.)", data_cuts, "colz");  
      c1_recon_corr->cd(4);
      T->Draw("P.kin.primary.W:P.gtr.dp>>H_shms_W_vs_delta(100, 8, 12, 100,0.9,1.)", data_cuts, "colz");  
      ...
      fsimc->cd();
      c1_recon_corr->cd(5);
      SNT->Draw("W:e_xptar>>H_shms_W_vs_xptar_simc(100, -0.015, 0.015, 100,0.9,1.)", simc_cuts, "colz");  
      c1_recon_corr->cd(6);
      SNT->Draw("W:e_yptar>>H_shms_W_vs_yptar_simc(100, -0.015, 0.015, 100,0.9,1.)", simc_cuts, "colz");  
      c1_recon_corr->cd(7);
      SNT->Draw("W:e_ytar>>H_shms_W_vs_ytar_simc(100, -1.5, 1.5, 100,0.9,1.)", simc_cuts, "colz");  
      c1_recon_corr->cd(8);
      SNT->Draw("W:e_delta>>H_shms_W_vs_delta_simc(100, 8, 12, 100,0.9,1.)", simc_cuts, "colz");  
      


      // missing momentum
      TCanvas *c2_recon_corr = new TCanvas("Pm_recon_corr", "Pm recons correlations", 1500, 700);      
      c2_recon_corr->Divide(4,2);

      // DATA
      fdata->cd();
      c2_recon_corr->cd(1);
      T->Draw("H.kin.secondary.pmiss:P.gtr.th>>H_shms_Pm_vs_xptar(100, -0.015, 0.015, 100,-0.01,0.05)", data_cuts, "colz");  
      c2_recon_corr->cd(2);
      T->Draw("H.kin.secondary.pmiss:P.gtr.ph>>H_shms_Pm_vs_yptar(100, -0.015, 0.015, 100,-0.01,0.05)", data_cuts, "colz");  
      c2_recon_corr->cd(3);
      T->Draw("H.kin.secondary.pmiss:P.gtr.y>>H_shms_Pm_vs_ytar(100, -1.5, 1.5, 100,-0.01,0.05)", data_cuts, "colz");  
      c2_recon_corr->cd(4);
      T->Draw("H.kin.secondary.pmiss:P.gtr.dp>>H_shms_Pm_vs_delta(100, 8, 12, 100,-0.01,0.05)", data_cuts, "colz");  
      ...
      fsimc->cd();
      c2_recon_corr->cd(5);
      SNT->Draw("Pm:e_xptar>>H_shms_Pm_vs_xptar_simc(100, -0.015, 0.015, 100,-0.01,0.05)", simc_cuts, "colz");  
      c2_recon_corr->cd(6);
      SNT->Draw("Pm:e_yptar>>H_shms_Pm_vs_yptar_simc(100, -0.015, 0.015, 100,-0.01,0.05)", simc_cuts, "colz");  
      c2_recon_corr->cd(7);
      SNT->Draw("Pm:e_ytar>>H_shms_Pm_vs_ytar_simc(100, -1.5, 1.5, 100,-0.01,0.05)", simc_cuts, "colz");  
      c2_recon_corr->cd(8);
      SNT->Draw("Pm:e_delta>>H_shms_Pm_vs_delta_simc(100, 8, 12, 100,-0.01,0.05)", simc_cuts, "colz");
    ...
    if(i==29) {
      ...
      */
      ...
      fdata->cd();
      c1_Pm_vs_recon->cd(1);
      T->Draw("H.kin.secondary.pmiss:P.gtr.ph>>H_Pm_vs_yptar(100, -0.02, 0.02, 100,-0.01,0.05)", data_cuts, "colz");  
      c1_Pm_vs_recon->cd(2);
      T->Draw("H.kin.secondary.Prec_x:P.gtr.ph>>H_Pmx_vs_yptar(100, -0.02, 0.02, 100,-0.1,0.1)", data_cuts, "colz");  
      c1_Pm_vs_recon->cd(3);
      T->Draw("H.kin.secondary.Prec_y:P.gtr.ph>>H_Pmy_vs_vs_yptar(100, -0.02, 0.02, 100,-0.1,0.1)", data_cuts, "colz");  
      c1_Pm_vs_recon->cd(4);
      T->Draw("H.kin.secondary.Prec_z:P.gtr.ph>>H_Pmz_vs_yptar(100, -0.02, 0.02, 100,-0.1,0.1)", data_cuts, "colz");  
      ...
      fsimc->cd();
      c1_Pm_vs_recon->cd(5);
      SNT->Draw("Pm:e_yptar>>H_Pm_vs_yptar_simc(100, -0.02, 0.02, 100,-0.01,0.05)", simc_cuts, "colz");  
      c1_Pm_vs_recon->cd(6);
      SNT->Draw("Pmx:e_yptar>>H_Pmx_vs_yptar_simc(100, -0.02, 0.02, 100,-0.1,0.1)", simc_cuts, "colz");  
      c1_Pm_vs_recon->cd(7);
      SNT->Draw("Pmy:e_yptar>>H_Pmy_vs_yptar_simc(100, -0.02, 0.02, 100,-0.1,0.1)", simc_cuts, "colz");  
      c1_Pm_vs_recon->cd(8);
      SNT->Draw("Pmz:e_yptar>>H_Pmz_vs_yptar_simc(100, -0.02, 0.02, 100,-0.1,0.1)", simc_cuts, "colz");  
