...
def make_final_summary():
    ...
    ofile.write('# \n'
                ...
                '# sigma_raw_per_nucleon   : corrected yield (yield_corr) normalized by (total_charge, transpacency, area_density(g/cm2) (per nucleons))\n'
                '# sigma_raw_per_nucleus   : corrected yield (yield_corr) normalized by (total_charge, transpacency, area_density (per nucleus))\n'
...
#    ofile.write('target,kin,beam_time,avg_current,total_charge,yield,yield_err,yield_corr,yield_corr_err,yield_norm,yield_norm_err,tgt_thick,T,N,Z,A\n')  original
    ofile.write('target,kin,beam_time,avg_current,total_charge,yield,yield_err,yield_eff,yield_eff_err,yield_corr,yield_corr_err,sigma_raw_per_nucleon,sigma_raw_per_nucleon_err,sigma_raw_per_nucleus,sigma_raw_per_nucleus_err,tgt_thick,tgt_thick_corr,T,N,Z,A\n') 
    ...
    for idx in np.arange(len(target)):
        ...
        for jdx in np.arange(len(kin)):
            ...
            sigma_raw_per_nucleus   =   sigma_raw_per_nucleon * A  # counts / (mC * g/cm^2 * nucleus)
    ...
    write_double_ratio(ofname, 'cafe_ratios_%s.csv'%(npass))
...
def write_double_ratio(ifname='', ofname=''):
    ...
    ofile.write('# \n'
                ...
                '# singleR_per_nucleus_err  : uncertainty in single ratio per nucleus \n'
                '# doubleR_per_nucleon      : double ratio of target A(SRC/MF) relative to C12 (SRC/MF) \n'
                '# doubleR_per_nucleon_err  : uncertainty in double ratio \n'
                '# doubleR_per_nucleus      : double ratio of target A(SRC/MF) relative to C12 (SRC/MF) \n'
                '# doubleR_per_nucleus_err  : uncertainty in double ratio \n'
