...
def make_final_summary_mod():
    ...
    for idx in np.arange(len(target)):
        ...
        for jdx in np.arange(len(kin)):
            ...
            print('************************')
            print('')
            print('yield_per_run: ', df['real_Yield'].to_numpy())
            print('yield = SUM[yield_per_run]: %.1f  --> (raw counts integrated over Pm) with all data-analysis cuts applied' % real_Yield_total)
            print('')
            print('////////////////////////////')
            print('// efficiency corrections //')
            print('////////////////////////////')
            print('')
            print('NOTE: The HMS/SHMS tracking efficiencies are averaged over all runs and then applied on a run-by-run basis')
            print('      The proton transmission factor gets applied as an overall correction factor (after summation of Y_per_run)')
            print('')
            print('htrk_eff_avg     : %.3f +/- %.3E'%(1/f1, hms_trk_eff_err_v2))
            print('(hdid: %.1f, hshould: %.1f)'%(h_did, h_should))
            print('etrk_eff_avg     : %.3f +/- %.3E'%(1/f2, shms_trk_eff_err_v2))
            print('(edid: %.1f, eshould: %.1f)'%(p_did, p_should))
            print('tLT_avg          : %.3f'%(1/f3))
            print('emult_trk_eff_avg: %.3f'%(1/f4))
            print('pTransm          : %.3f'%(1/f5))
            print('eff_total        : %.3f'%((1./f1) * (1./f2) * (1./f3) * (1./f4) * (1./f5)))
            print('')
            print('yield_eff_per_run = yield_per_run / (htrk_eff_avg * etrk_eff_avg * tLT_avg * emult_trk_eff_avg) ')
            print('yield_eff = SUM[ yield_eff_per_run ] * (1 / pTransm) = SUM [yield_eff_per_run / %.3f] * 1/%.3f' % ( ((1./f1) * (1./f2) * (1./f3) * (1./f4)), 1./f5 ) )
            print('yield_eff = %.1f -->  yield corrected for inefficiencies (hms/shms tracking + live_time + proton absorption) ' % (real_Yield_eff_total))
            print('')
            print('')
            if(target[idx]=='B10' or target[idx]=='B11' or target[idx]=='Ca40' or target[idx]=='Ca48'):
                print('////////////////////////////////////////////////////////////////////////////////')
            ...
            if(target[idx]=='B10' or target[idx]=='B11'):
                ...
                print('------------------------------------------------------------')
                print('')
                print('a. calculate the c12 density contribution in boron carbide (B4C) ')
                print('(subtracted the boron contribution from the b4c density)')
                print('')
                ...
                print('yield_corr = yield_eff - yield_eff_c12 * (c12_density_b4c/c12_density) * (Q_%s / Q_C12)' % (target[idx]))
            ...
            if(target[idx]=='Ca40'):
                ...
                print('')
                print('yield_corr_per_run = yield_eff_per_run * ca40_cntm_eff_per_run  ')
                print('yield_corr = SUM_[yield_corr_per_run] * 1/pTransm = %.3f * 1./%.3f' % (real_Yield_corr_total, (1/f5) )  )
                print('yield_corr = %.3f --> yield_eff corrected for external/internal impurities (if any) ' % (real_Yield_corr_total))
            ...
            if(target[idx]=='Ca48'):
                ...
                print('')
                print('yield_corr_per_run = yield_eff_per_run * ca48_cntm_eff_per_run  ')
                print('yield_corr = SUM_[yield_corr_per_run] * 1/pTransm = %.3f * 1./%.3f (apply overall proton transmission factor to SUM)' % (real_Yield_corr_total, (1/f5) )  )
                print('yield_corr = %.3f --> yield_eff corrected for external/internal impurities (if any)' % (real_Yield_corr.sum() * f5))
                print('')
                print('---------------------------------------')
                print(' apply Ca48 "Ca40 impurity" correction ')
                print('---------------------------------------')
                print('')
                print(' NOTE: Ca48 is only ~90.5 % pure, the remaining is Ca40. The 48Ca target is 1051 mg/cm2.' )
                print('')
                ...
                print('')
                print('yield_corr = yield_corr - yield_corr_ca40 * (ca40_cntm / ca40_density) * (Q_ca48 / Q_ca40) ')
                print('yield_corr = %.3f - %.3f * (%.5f / %.5f) * (%.3f / %.3f)' % (real_Yield_corr.sum() * f5, real_Yield_corr_total_Ca40[kin[jdx]], ca40_cntm, ca40_density, total_charge, Q_Ca40[kin[jdx]]) )
                print('yield_corr = %.3f --> yield_eff corrected for external/internal impurities (if any)' % real_Yield_corr_total)
            print('')
            print('')
...
def write_ratios(ifname='', ofname=''):
    ...
    # set output file to write CaFe triple [Ca40 Ca48 Fe54] / Ca40 single SRC ratios (C. Yero added on Sep 20)
    ofile3 = open('cafe_triplet_to40.csv', 'w+')
    ofile3.write('# CaFe Triplet Numerical Ratios (%s) \n'%(npass))
    ofile3.write('# \n'
                '# Header Definitions: \n'
                '# target       : target A used in single ratio \n'
                '# singleR_A_ca40_mf   : single ratio of target A_MF / Ca40_MF (per proton) \n'
                '# singleR_A_ca40_src  : single ratio of target A_SRC / Ca40_SRC (per proton) \n'
                '# doubleR      : double ratio of target A(SRC/MF) relative to Ca40 (SRC/MF) \n'
                '# _stat_err    : absolute statistical uncertainty \n'
                '# _norm_syst_err : absolute systematic error due to normalization correction factors (a) hms/shms track efficiencies (b) live time (c) proton transmission (d) ca 40/48 cntm)\n'
                '# _RC_syst_err   : absolute systematic error due to radiative corrections \n'
                '# _cut_syst_err  : absolute systematic error due to cut sensitivity \n'
    ...
    ofile3.write('target,singleR_A_ca40_mf,singleR_A_ca40_mf_stat_err,singleR_A_ca40_mf_norm_syst_err,singleR_A_ca40_mf_RC_syst_err,singleR_A_ca40_mf_cut_syst_err,singleR_A_ca40_mf_syst_err,singleR_A_ca40_mf_tot_err,singleR_A_ca40_src,singleR_A_ca40_src_stat_err,singleR_A_ca40_src_norm_syst_err,singleR_A_ca40_src_RC_syst_err,singleR_A_ca40_src_cut_syst_err,singleR_A_ca40_src_syst_err,singleR_A_ca40_src_tot_err,doubleR,doubleR_stat_err,doubleR_norm_syst_err,doubleR_RC_syst_err,doubleR_cut_syst_err,doubleR_syst_err,doubleR_tot_err,N,Z,A,NoZ,NmZoA\n') 
    ...
    rad_corr_ratio_ca40 = (rad_corr_mf_ca40/rad_corr_src_ca40)
    ...
    # re-define the single ratio for Ca40 (just to make it easier later on when dividing by Ca40 for the triplet) C. Yero, Sep 20
    singleR_per_proton_Ca40               = (src_sigma_raw_per_proton_Ca40.values/mf_sigma_raw_per_proton_Ca40.values) * rad_corr_ratio_ca40  # apply radiative correction
    singleR_per_proton_Ca40_RC_syst_err   = singleR_per_proton_Ca40 * rad_corr_ratio_rel_err                                                  # absolute syst. error due to rad corr ratio
    singleR_per_proton_Ca40_stat_err      = singleR_per_proton_Ca40 * np.sqrt(stat_rel_err_src_ca40**2 +  stat_rel_err_mf_ca40**2)            # absolute stat. error 
    singleR_per_proton_Ca40_norm_syst_err = singleR_per_proton_Ca40 * np.sqrt(norm_syst_rel_err_src_ca40**2 +  norm_syst_rel_err_mf_ca40**2)  # absolute norm syst. error 
    ...
    # initialize zero arrays to be filled in depending on relative error value (to Ca40) - C. Yero Sep 20
    singleR_A_ca40_mf               = np.zeros([3])
    singleR_A_ca40_mf_RC_syst_err   = np.zeros([3])
    singleR_A_ca40_mf_stat_err      = np.zeros([3])
    singleR_A_ca40_mf_norm_syst_err = np.zeros([3])
    singleR_A_ca40_mf_cut_syst_err  = np.zeros([3])

    singleR_A_ca40_mf_syst_err      = np.zeros([3])
    singleR_A_ca40_mf_tot_err       = np.zeros([3])
    ...
    # apply radiative correction factor to single ratio A/Ca40 - C. Yero Sep 20
    singleR_A_ca40_mf[0]               =  ( float(mf_sigma_raw_per_proton_Ca40) / float(mf_sigma_raw_per_proton_Ca40) ) * (rad_corr_mf_ca40 / rad_corr_mf_ca40)
    singleR_A_ca40_mf[1]               =  ( float(mf_sigma_raw_per_proton_Ca48) / float(mf_sigma_raw_per_proton_Ca40) ) * (rad_corr_mf_ca40 / rad_corr_mf_ca48)
    singleR_A_ca40_mf[2]               =  ( float(mf_sigma_raw_per_proton_Fe54) / float(mf_sigma_raw_per_proton_Ca40) ) * (rad_corr_mf_ca40 / rad_corr_mf_fe54)


    
    # RC systematic error (to Ca48) : need to figure this out (what is the relative error on the radiative correction factor for (Ca40 Ca48 Fe54) / Ca48 single ratio?
    singleR_A_ca48_mf_RC_syst_err[0]       = 0  # singleR_A_ca48_mf[0] * rad_corr_ratio_mf_rel_err ???
    singleR_A_ca48_mf_RC_syst_err[1]       = 0
    singleR_A_ca48_mf_RC_syst_err[2]       = singleR_A_ca48_mf[2] * 0.025   # (Larry) suggests we add 2.5% relative systematic uncertainty to Fe for the rad corr. 
    ...
    # calculate absolute error on triplet ratio due to transparency (to Ca40)
    singleR_A_ca40_mf_T_syst_err      = singleR_A_ca40_mf  *  singleR_mf_triplet_T_syst_rel_err_40 

    # calculate absolute statistical, normalization and cut systematic absolute error (to Ca48)
    ...
    # calculate absolute statistical, normalization and cut systematic absolute error (to Ca40)  -C. Yero Sep 20
    singleR_A_ca40_mf_stat_err[0]      = singleR_A_ca40_mf[0] * np.sqrt(stat_rel_err_mf_ca40**2 +  stat_rel_err_mf_ca40**2)
    singleR_A_ca40_mf_stat_err[1]      = singleR_A_ca40_mf[1] * np.sqrt(stat_rel_err_mf_ca48**2 +  stat_rel_err_mf_ca40**2)
    singleR_A_ca40_mf_stat_err[2]      = singleR_A_ca40_mf[2] * np.sqrt(stat_rel_err_mf_fe54**2 +  stat_rel_err_mf_ca40**2)

    singleR_A_ca40_mf_norm_syst_err[0]      = singleR_A_ca40_mf[0] * np.sqrt(norm_syst_rel_err_mf_ca40**2 +  norm_syst_rel_err_mf_ca40**2)
    singleR_A_ca40_mf_norm_syst_err[1]      = singleR_A_ca40_mf[1] * np.sqrt(norm_syst_rel_err_mf_ca48**2 +  norm_syst_rel_err_mf_ca40**2)
    singleR_A_ca40_mf_norm_syst_err[2]      = singleR_A_ca40_mf[2] * np.sqrt(norm_syst_rel_err_mf_fe54**2 +  norm_syst_rel_err_mf_ca40**2)

    singleR_A_ca40_mf_cut_syst_err[0]  = singleR_A_ca40_mf[0] * singleR_mf_triplet_cut_syst_rel_err[0]
    singleR_A_ca40_mf_cut_syst_err[1]  = singleR_A_ca40_mf[1] * singleR_mf_triplet_cut_syst_rel_err[1]
    singleR_A_ca40_mf_cut_syst_err[2]  = singleR_A_ca40_mf[2] * singleR_mf_triplet_cut_syst_rel_err[2]

    
    #------ add errors in quadrature (for ratio to Ca48) ----
    ...
    #------ add errors in quadrature (for ratio to Ca40) ----
    singleR_A_ca40_mf_syst_err[0] = np.sqrt(singleR_A_ca40_mf_norm_syst_err[0]**2 + singleR_A_ca40_mf_RC_syst_err[0]**2 + singleR_A_ca40_mf_cut_syst_err[0]**2 + singleR_A_ca40_mf_T_syst_err[0]**2)
    singleR_A_ca40_mf_tot_err[0]  = np.sqrt(singleR_A_ca40_mf_stat_err[0]**2 + singleR_A_ca40_mf_syst_err[0]**2)

    singleR_A_ca40_mf_syst_err[1] = np.sqrt(singleR_A_ca40_mf_norm_syst_err[1]**2 + singleR_A_ca40_mf_RC_syst_err[1]**2 + singleR_A_ca40_mf_cut_syst_err[1]**2 + singleR_A_ca40_mf_T_syst_err[1]**2)
    singleR_A_ca40_mf_tot_err[1]  = np.sqrt(singleR_A_ca40_mf_stat_err[1]**2 + singleR_A_ca40_mf_syst_err[1]**2)

    singleR_A_ca40_mf_syst_err[2] = np.sqrt(singleR_A_ca40_mf_norm_syst_err[2]**2 + singleR_A_ca40_mf_RC_syst_err[2]**2 + singleR_A_ca40_mf_cut_syst_err[2]**2 + singleR_A_ca40_mf_T_syst_err[2]**2)
    singleR_A_ca40_mf_tot_err[2]  = np.sqrt(singleR_A_ca40_mf_stat_err[2]**2 + singleR_A_ca40_mf_syst_err[2]**2)
    ...
    #-------------------------------------------------
    # Calculate CaFe Triplet Ratio relative to Ca40  (C. Yero, added on Sep 20)
    #
    #--- A_SRC / Ca40_SRC (per proton) ---
    #-------------------------------------------------
    ...
    singleR_A_ca40_src_RC_syst_err   = np.zeros([3])
    singleR_A_ca40_src_stat_err      = np.zeros([3])
    singleR_A_ca40_src_norm_syst_err = np.zeros([3])
    singleR_A_ca40_src_cut_syst_err  = np.zeros([3])

    singleR_A_ca40_src_syst_err      = np.zeros([3])
    singleR_A_ca40_src_tot_err       = np.zeros([3])

    
    # apply radiative correction factor to single ratio A/Ca40 (C. Yero, added on Sep 20)
    
    singleR_A_ca40_src[0]               =  ( float(src_sigma_raw_per_proton_Ca40) / float(src_sigma_raw_per_proton_Ca40) ) * (rad_corr_src_ca40 / rad_corr_src_ca40)
    singleR_A_ca40_src[1]               =  ( float(src_sigma_raw_per_proton_Ca48) / float(src_sigma_raw_per_proton_Ca40) ) * (rad_corr_src_ca40 / rad_corr_src_ca48)
    singleR_A_ca40_src[2]               =  ( float(src_sigma_raw_per_proton_Fe54) / float(src_sigma_raw_per_proton_Ca40) ) * (rad_corr_src_ca40 / rad_corr_src_fe54)
    ...
    # calculate absolute errors on stats, norm_syst, cut_syst
    singleR_A_ca40_src_stat_err[0]      = singleR_A_ca40_src[0] * np.sqrt(stat_rel_err_src_ca40**2 +  stat_rel_err_src_ca40**2)
    singleR_A_ca40_src_stat_err[1]      = singleR_A_ca40_src[1] * np.sqrt(stat_rel_err_src_ca48**2 +  stat_rel_err_src_ca40**2)
    singleR_A_ca40_src_stat_err[2]      = singleR_A_ca40_src[2] * np.sqrt(stat_rel_err_src_fe54**2 +  stat_rel_err_src_ca40**2)

    singleR_A_ca40_src_norm_syst_err[0]      = singleR_A_ca40_src[0] * np.sqrt(norm_syst_rel_err_src_ca40**2 +  norm_syst_rel_err_src_ca40**2)
    singleR_A_ca40_src_norm_syst_err[1]      = singleR_A_ca40_src[1] * np.sqrt(norm_syst_rel_err_src_ca48**2 +  norm_syst_rel_err_src_ca40**2)
    singleR_A_ca40_src_norm_syst_err[2]      = singleR_A_ca40_src[2] * np.sqrt(norm_syst_rel_err_src_fe54**2 +  norm_syst_rel_err_src_ca40**2)

    singleR_A_ca40_src_cut_syst_err[0]  = singleR_A_ca40_src[0] * singleR_src_triplet_cut_syst_rel_err_40[0]
    singleR_A_ca40_src_cut_syst_err[1]  = singleR_A_ca40_src[1] * singleR_src_triplet_cut_syst_rel_err_40[1]
    singleR_A_ca40_src_cut_syst_err[2]  = singleR_A_ca40_src[2] * singleR_src_triplet_cut_syst_rel_err_40[2]

    #------ add errors in quadrature ----
    singleR_A_ca40_src_syst_err[0] = np.sqrt(singleR_A_ca40_src_norm_syst_err[0]**2 + singleR_A_ca40_src_RC_syst_err[0]**2 + singleR_A_ca40_src_cut_syst_err[0]**2 + singleR_A_ca40_src_T_syst_err[0]**2 )
    singleR_A_ca40_src_tot_err[0]  = np.sqrt(singleR_A_ca40_src_stat_err[0]**2 + singleR_A_ca40_src_syst_err[0]**2)

    singleR_A_ca40_src_syst_err[1] = np.sqrt(singleR_A_ca40_src_norm_syst_err[1]**2 + singleR_A_ca40_src_RC_syst_err[1]**2 + singleR_A_ca40_src_cut_syst_err[1]**2 + singleR_A_ca40_src_T_syst_err[1]**2 )
    singleR_A_ca40_src_tot_err[1]  = np.sqrt(singleR_A_ca40_src_stat_err[1]**2 + singleR_A_ca40_src_syst_err[1]**2)

    singleR_A_ca40_src_syst_err[2] = np.sqrt(singleR_A_ca40_src_norm_syst_err[2]**2 + singleR_A_ca40_src_RC_syst_err[2]**2 + singleR_A_ca40_src_cut_syst_err[2]**2 + singleR_A_ca40_src_T_syst_err[2]**2 )
    singleR_A_ca40_src_tot_err[2]  = np.sqrt(singleR_A_ca40_src_stat_err[2]**2 + singleR_A_ca40_src_syst_err[2]**2)
    ...
    #---------------------------------------------------------
    # ---- CALCULATE DOUBLE RATIOS A_SRC/MF / Ca40_SRC/MF ----
    #---------------------------------------------------------
    # C. Yero added on Sep 20
    ...
    # initialize zero arrays to be filled in depending on relative error value
    doubleRt_40               = np.zeros([3])
    doubleRt_stat_err_40      = np.zeros([3])
    doubleRt_RC_syst_err_40   = np.zeros([3])
    doubleRt_norm_syst_err_40 = np.zeros([3])
    doubleRt_cut_syst_err_40  = np.zeros([3])

    doubleRt_RC_corr_factor_rel_err_40 = np.zeros([3])
    
    # loop over rad corr factor and apply relative error on rad corr (C. Yero Sep 20, needs to be updated )
    for i in range(3):
        if doubleRt_RC_corr_factor_40[i] == 1:
            doubleRt_RC_corr_factor_rel_err_40[i] = 0.01  # default
            #print('doubleR_RC_corr_factor_rel_err = ',doubleRt_RC_corr_factor_rel_err[i])
        else:
            doubleRt_RC_corr_factor_rel_err_40[i] =  np.abs(1.-doubleRt_RC_corr_factor_40[i]) * 0.20
            #print('doubleR_RC_corr_factor_rel_err = ',doubleRt_RC_corr_factor_rel_err[i])
        
    doubleRt_40                = singleR_per_proton[4:7] / singleR_per_proton_Ca40
    doubleRt_stat_err_40       = doubleRt_40 * np.sqrt( (singleR_per_proton_stat_err[4:7]/singleR_per_proton[4:7])**2 +  (singleR_per_proton_Ca40_stat_err/singleR_per_proton_Ca40)**2 )
    doubleRt_RC_syst_err_40    = doubleRt_40 * doubleRt_RC_corr_factor_rel_err_40
    doubleRt_norm_syst_err_40  = doubleRt_40 * np.sqrt( (singleR_per_proton_norm_syst_err[4:7]/singleR_per_proton[4:7])**2 + (singleR_per_proton_Ca40_norm_syst_err/singleR_per_proton_Ca40)**2  )
    doubleRt_cut_syst_err_40   = doubleRt_40 * doubleRt_cut_syst_rel_err_40

    #print('doubleR_RC_corr_factor = ',doubleRt_RC_corr_factor)
    #print('doubleR_RC_corr_factor_rel_err = ',doubleRt_RC_corr_factor_rel_err)

    # -------- add errors in quadrature
    doubleRt_syst_err_40 = np.sqrt(doubleRt_norm_syst_err_40**2 + doubleRt_RC_syst_err_40**2 + doubleRt_cut_syst_err_40**2)
    doubleRt_tot_err_40 = np.sqrt(doubleRt_stat_err_40**2 + doubleRt_syst_err_40**2)
    #-----------------------------------

    
    #-------------------------------------------------------
    #
    # Write CaFe Triplet Ratios (to Ca48) to Numerical File
    #
    #-------------------------------------------------------
    ...
    #-------------------------------------------------------
    #
    # Write CaFe Triplet Ratios (to Ca40) to Numerical File
    #
    #-------------------------------------------------------
    ...
    for i in np.arange(len(targ)):
        ...
        ofile3.write('%s,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.3E,%.1f,%.1f,%.1f,%.3f,%.3f\n' % (targ[i],
                                                       ...
                                                       doubleRt_40[i],doubleRt_stat_err_40[i],doubleRt_norm_syst_err_40[i],doubleRt_RC_syst_err_40[i],doubleRt_cut_syst_err_40[i],doubleRt_syst_err_40[i],doubleRt_tot_err_40[i],                                                                        
                                                       Nt[i], Zt[i], At[i], NoZ_t[i], NmZoA_t[i]) ) 
