void make_plots_deut(){
  ...
  for(int i=0; i<nplots; i++){
    ...
    if(i==0) {
      ...
      fdata->cd();
      T->Draw("P.gtr.p>>H_shms_kf(100,7.5,8.5)", data_cuts, "normhistE");
      fsimc->cd();
      SNT->Draw("e_pf/1000.>>H_shms_kf_simc(100,7.5,8.5)", simc_cuts, "normhistEsames");
    ...
    if(i==6) {
      ...
      c[i] = new TCanvas(Form("c%i",i), "", 900, 700);
      //fsimc->cd();
      //SNT->Draw("W>>H_W_simc(100,0.8,1.1)", simc_cuts, "normhistE");
      fdata->cd();
      T->Draw("P.kin.primary.W>>H_W(100,0.8,1.1)", data_cuts, "normhistE");
      fsimc->cd();
      SNT->Draw("W>>H_W_simc(100,0.8,1.1)", simc_cuts, "normhistEsames");
    }
    ...
    if(i==8) {
      c[i] = new TCanvas(Form("c%i",i), "", 900, 700);
      fdata->cd();
      T->Draw("P.kin.primary.Q2>>H_Q2(100,0,6)", data_cuts, "normhistE");
      fsimc->cd();
      SNT->Draw("Q2>>H_Q2_simc(100,0,6)", simc_cuts, "normhistEsames");
    }

    if(i==9) {
      c[i] = new TCanvas(Form("c%i",i), "", 900, 700);
      fdata->cd();
      T->Draw("P.kin.primary.scat_ang_deg>>H_the(100,12,16)", data_cuts, "normhistE");
      fsimc->cd();
      SNT->Draw("theta_e*180/3.14159265359>>H_the_simc(100,12,16)", simc_cuts, "normhistEsames");
    ...
    if(i==10) {
      c[i] = new TCanvas(Form("c%i",i), "", 900, 700);
      fdata->cd();
      T->Draw("P.kin.primary.nu>>H_nu(200,0.4,2.4)", data_cuts, "normhistE"); 
      fsimc->cd();
      SNT->Draw("nu>>H_nu_simc(200,0.4,2.4)", simc_cuts, "normhistEsames");
    }
