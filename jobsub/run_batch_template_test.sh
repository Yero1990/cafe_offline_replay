...
#shell script suffix (_data.sh or _simc.sh)
ana_type=${0##*_}
ana_type=${ana_type%%.sh}     

if [ "${ana_type}" = "data" ]; then
    
    #echo "ana_type ---> ${ana_type}"
    Workflow="cafe_official_${ana_type}_${USER}" # Change this as desired
    ...
    # generic simc input file name
    script_path="/w/hallc-scshelf2102/c-cafe-2022/cyero/cafe_offline_replay/"
    target="d2"
    analysis_cut="MF"
	...
	case $yn in
		...
		i=-1
		(
		    # change directory to where command file is
		    cd ${script_path} 

		    batch="${USER}_Job.txt"
		    echo "Running ${batch}"
		    cp /dev/null ${batch}
		    echo "PROJECT: c-comm2017" >> ${batch}
		    echo "TRACK: debug" >> ${batch}
		    #echo "TRACK: analysis" >> ${batch}
		    echo "JOBNAME: ${test_job}" >> ${batch}
		    echo "DISK_SPACE: 3 GB" >> ${batch}
		    echo "MEMORY: 3000 MB" >> ${batch}
		    echo "CPU: 1" >> ${batch}
		    echo "COMMAND: ${script_path}analyze_cafe_data.sh ${target} ${analysis_cut}" >> ${batch}
		    echo "MAIL: ${USER}@jlab.org" >> ${batch}
		    echo "Submitting batch"
		    # swif2 is now used for job submission, we use our old jsub style scripts. The argument set to "LTSep" currently is the workflow. Change this if you want.
		    eval "swif2 add-jsub ${Workflow} -script ${batch} 2>/dev/null" # Swif2 job submission, uses old jsub scripts
